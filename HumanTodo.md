...
- [ ] rimapi-room-entry-cells [2026-05-27] #rimapi #construction #willie Extend `rimapi-room-detail-read` (or land sibling endpoint) to emit per-room boundary door/door-opening cells. Source for Willie `AnchorInventory.EntryCells[]` (FORK3 batch path-cost `to` set) and the `basic_shelter` `missing_door` rule. Fold into `rimapi-room-detail-read` if its scope already covers boundary cells. [plan](.plans/willie-briefing-schema.md)

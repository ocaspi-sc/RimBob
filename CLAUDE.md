# RimAI — Claude Instructions

This file is loaded by Claude Code at the start of every session. It defines how Claude should behave in this project.
...
| Topic | Doc to update |
|---|---|
| High-level principles, design goals | `DESIGN.md` |
| Build order, milestones | `ROADMAP.md` |
| Actionable tasks | `TODO.md` |
| Minister shape, rules system, rule refinement | `design/ministers.md` |
...
| Code structure, stack, interfaces | `design/architecture.md` |
| RIMAPI endpoints, conventions, controller catalogue | [`design/rimapi.md`](design/rimapi.md) |
| Specific minister | `design/ministers/<name>.md` |
| HTN, planning *(deferred — Auto epic)* | `design/planning.md` |
| Labor / assignment solver *(deferred — Auto epic)* | `design/ministers/labor.md` |

If a decision affects multiple docs, update the most specific one and add a cross-reference in `DESIGN.md`'s decision log.
...
- **At session start:** read `DESIGN.md`, `TODO.md`, and any doc relevant to the session's focus.
- **At session end:** offer to update `TODO.md` with any new tasks uncovered.
...
- Every new minister gets its own directory under `Ministers/<Name>/`.
- `Rules.cs` is the first file in every minister directory. It must compile and have tests before the LLM is wired.
- Fixture JSON files live in `Tests/<MinisterName>/Fixtures/`.
...
When in doubt about architecture, read `DESIGN.md` first.

---

## Important constraints

- **Suggest by default; autonomy is per-minister and opt-in.** MVP ships with every advisor in `Suggest`; only `Suggest` is honored. See `design/advice.md`.

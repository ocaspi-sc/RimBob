...
| Assisted Apply is player-confirmed, not Auto | Some low-risk advice actions are more useful when the player can apply them from the dashboard. MVP may support a narrow allowlist such as `unforbid` and deterministic map designations after the player clicks Apply. Ministers and LLMs still emit structured actions, never raw endpoints; Host validates current state, applies exactly one non-pawn operation, reads back/logs the result, and leaves pawn allocation, HTN, bills, zones, schedules, and autonomous execution to later slices. See [`design/advice.md`](design/advice.md), [`design/dashboard.md`](design/dashboard.md), [`design/architecture.md`](design/architecture.md), and [`design/planning.md`](design/planning.md). |

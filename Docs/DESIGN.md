...
| Local launcher gives the Host a taskbar-visible server window | Normal local runs should leave an obvious Windows taskbar affordance so the player can find RimBob while playing. `run-rimbob.ps1` therefore starts `RimBob.Host` in a minimized PowerShell window named `RimBob Server`, and that window closes when RimBob exits. `-Foreground` keeps the old attached-console behavior for debugging and verification. See [`design/architecture.md`](design/architecture.md). |
...
| Dashboard console data is split across SYSTEM, INFO, and ANALYTICS | SYSTEM is the operations/debug console for raw runtime health, endpoints, SSE diagnostics, traces, and logs. INFO is reference-only: glossary terms and a scope guide. ANALYTICS owns derived live signals such as high-level SSE health, session load, colony pressure, advice mix, and candidate analytics. SYSTEM and ANALYTICS may read the same bounded inputs, but INFO should not carry live metrics. See [`design/dashboard.md`](design/dashboard.md). |

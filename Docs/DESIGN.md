...
| M3 Food feeder ships as rules-first plus LLM escalation | Food is the first full feeder minister. It derives a focused `FoodBriefing`, emits `AdviceItem`s and `AgentFlag`s, escalates ambiguous crop/hunt/freezer tradeoffs to Gemini, and feeds active Medium+ flags into the Mayor through `CabinetCycle` / `FlagChannel`. Food remains suggest-only: advice steps are rendered, not executed. See [`design/ministers/food.md`](design/ministers/food.md), [`design/communication.md`](design/communication.md), and [`design/dashboard.md`](design/dashboard.md). |
...
| Advice wire fields are execution-facing | `steps[]` uses `instruction` plus optional `quantity`, `owner`, `work_type`, `skill`, `reason`, and `icon` so raw minister output and the dashboard can feed a future execution layer without translation. The tolerant parser may still accept old `resource_requests[]`, `suggested_actions[]`, `what`, and `why` model output as an input repair path. See [`design/advice.md`](design/advice.md) and [`design/dashboard.md`](design/dashboard.md). |

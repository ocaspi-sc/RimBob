...
| Food raw LLM output has a manual Codex fallback | When Gemini is unavailable or quota-limited, `POST /api/ministers/food/llm-output/manual` can ingest a raw Food JSON response generated outside the provider, record it as `Codex` / `codex-subagent`, parse it through the same Food response parser, and publish the resulting Food advice snapshot. This is a developer fallback only and remains suggest-only. See [`design/dashboard.md`](design/dashboard.md). |

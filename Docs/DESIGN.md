...
| Suggested action kind names should describe the UI/game operation | `suggested_actions[].kind` values should be explicit enough to stand alone in the dashboard and future Auto mapping. Prefer names like `mark_harvest`, `mark_hunt`, `place_blueprint`, `production_bill`, and `set_stockpile_zone` over terse or generic labels such as `build` or `note` when a structured operation exists. See [`design/advice.md`](design/advice.md). |
...
| Advice wire fields are execution-facing | `resource_requests[]` uses `request` and `reason`, not generic `what` / `why`, and `suggested_actions[]` uses `instruction`, not generic `what`. Raw minister output and the dashboard should expose fields that can later feed the execution layer without translation. The tolerant parser may still accept old `what` / `why` model output as an input repair path. See [`design/advice.md`](design/advice.md) and [`design/dashboard.md`](design/dashboard.md). |

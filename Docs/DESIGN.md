...
For the MVP, output is advice by default. Narrow player-confirmed Assisted Apply may execute allowlisted non-pawn RIMAPI writes for concrete advice steps; autonomous execution remains deferred.
...
4. **Transparent.** Every memo carries its rationale, ordered steps, and the briefing it was based on. A single dashboard view should explain what the cabinet is recommending and why.
...
| Advice uses one ordered step path | `AdviceItem` carries `steps[]` as the single player-facing action path instead of splitting one workflow across `resource_requests[]` and `suggested_actions[]`. Flags still carry `AgentFlag.Requests` for cross-minister urgency. Legacy raw outputs may be normalized into steps, but new prompts/specs emit steps directly. See [`design/advice.md`](design/advice.md), [`design/ministers.md`](design/ministers.md), and [`design/dashboard.md`](design/dashboard.md). |
...
| M3 Food feeder ships as rules-first plus LLM escalation | Food is the first full feeder minister. It derives a focused `FoodBriefing`, emits `AdviceItem`s and `AgentFlag`s, escalates ambiguous crop/hunt/freezer tradeoffs to Gemini, and feeds active Medium+ flags into the Mayor through `CabinetCycle` / `FlagChannel`. Food remains in `Suggest` mode: advice steps are rendered by default, with only future Assisted Apply able to execute allowlisted steps after a player click. See [`design/ministers/food.md`](design/ministers/food.md), [`design/communication.md`](design/communication.md), and [`design/dashboard.md`](design/dashboard.md). |
...
| Advice step kind names should describe the UI/game operation | `steps[].kind` values should be explicit enough to stand alone in the dashboard and future Auto mapping. Prefer names like `mark_harvest`, `mark_hunt`, `place_blueprint`, `production_bill`, `designate_zone`, and `set_stockpile_zone` over terse or generic labels such as `build` or `note` when a structured operation exists. See [`design/advice.md`](design/advice.md). |
...
| Advice wire fields are execution-facing | `steps[]` uses `instruction` plus optional `quantity`, `owner`, `work_type`, `skill`, `reason`, and `icon` so raw minister output and the dashboard can feed Assisted Apply or a future execution layer without translation. The tolerant parser may still accept old `resource_requests[]`, `suggested_actions[]`, `what`, and `why` model output as an input repair path. See [`design/advice.md`](design/advice.md) and [`design/dashboard.md`](design/dashboard.md). |
...
| Assisted Apply is player-confirmed, not Auto | Some low-risk advice steps are more useful when the player can apply them from the dashboard. MVP may support a narrow allowlist such as `unforbid` and deterministic map designations after the player clicks Apply. Ministers and LLMs still emit structured steps, never raw endpoints; Host validates current state, applies exactly one non-pawn operation, reads back/logs the result, and leaves pawn allocation, HTN, bills, zones, schedules, and autonomous execution to later slices. See [`design/advice.md`](design/advice.md), [`design/dashboard.md`](design/dashboard.md), [`design/architecture.md`](design/architecture.md), and [`design/planning.md`](design/planning.md). |

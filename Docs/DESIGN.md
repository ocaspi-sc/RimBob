...
| Mayor Agenda current/history are durable Host runtime state | Browser-side agenda caching is rejected as a confusing source of truth. `AgendaStore` persists the current Agenda and bounded history under ignored `var/agenda/` and loads it on Host startup, so `/api/agenda/latest` and SSE replay can recover the last good agenda after a restart or failed Mayor LLM call. → [`design/agenda.md`](design/agenda.md), [`design/dashboard.md`](design/dashboard.md) |
| Curated ColonyState latest snapshot is durable but stale-labeled | Host persists the latest full curated state-store aggregate root after a complete live RIMAPI refresh and may restore it before agenda bootstrap when RIMAPI is down. Restored state keeps the dashboard and briefings useful, but it is marked snapshot-sourced until a new live refresh succeeds; Assisted Apply still requires fresh live state. → [`design/state-store.md`](design/state-store.md), [`design/dashboard.md`](design/dashboard.md) |

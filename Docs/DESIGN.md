...
| Food snapshots include a deterministic current-state paragraph | Food advice should start with a short factual read of the food situation before listing action items. The paragraph is built from the Food briefing, not trusted to LLM prose, and should name concrete stores/growing/storage facts such as stored meals, raw or unclassified food, growing areas, crop progress, kitchen/storage, and confidence gaps. Advice items remain the executable reading surface. See [`design/ministers/food.md`](design/ministers/food.md), [`design/advice.md`](design/advice.md), and [`design/dashboard.md`](design/dashboard.md). |

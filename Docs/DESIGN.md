...
For the MVP, output is advice by default. Narrow player-confirmed Assisted Apply may execute allowlisted non-pawn RIMAPI writes for concrete advice actions; autonomous execution remains deferred.
...
**Suggest by default; autonomy is per-minister and opt-in.** MVP ships with every advisor in `Suggest` mode. Assisted Apply may let the player manually execute one allowlisted advice step, but it is not `Auto`. Graduating an advisor to `Auto` is a deliberate, per-minister event gated by track record + explicit player consent. → [`design/advice.md`](design/advice.md)

...
**Configuration.** Non-secret config lives in `appsettings.json` under the `RimAi` section (typed via `RimAiOptions`). Gemini secrets use one ordered list: `GEMINI_API_KEYS` (semicolon/comma/newline separated) or `RimAi.GeminiApiKeys` in gitignored `appsettings.Local.json`. `Program.cs` resolves that list during DI registration. `LlmClient` and `GeminiEmbedder` try the next configured key only for quota/rate-limit/key failures, never logging key values. `appsettings.json` and `appsettings.Development.json` are tracked, so never put secrets in those. `LlmClient` tolerates missing keys (constructor does not throw, `IsConfigured` is false, `PingAsync` returns false) so the Host can boot for `/api/health` smoke checks without credentials.

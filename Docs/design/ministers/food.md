...
Chef owns food-zone needs, not map coordinates. For `expand_growing_capacity`, Chef chooses the crop, tile count, urgency, and food-chain reason, then emits a typed `zone_requests[]` row routed to Willie for spatial placement. For missing reachable food storage, Chef emits a `zone_class: stockpile` request with a bounded tile count and `Foods` item-category filter instead of pretending a stockpile is a building request. Willie may return zone options, a no-fit reason, and a Willie-owned `create_growing_zone` or `create_stockpile_zone` Apply action for a concrete rectangular option, but Chef's outbound flag must stay read-only and must not bury zone placement in `attention[]`.
...
Growing-capacity requests should carry a typed `ZoneRequest` with `zone_class: growing`, crop def, tile count, terrain growability need, priority, and `requested_from: Willie`. Food-stockpile requests should carry `zone_class: stockpile`, tile count, priority, `requested_from: Willie`, and the `Foods` category filter. Chef owns why and how much food crop or food storage is needed; Willie owns whether there is a valid place to put it.

In MVP advice actions and flag requests are rendered by default. Assisted Apply may execute a narrow allowlist of Chef actions after player confirmation, such as `unforbid` known food stacks, `mark_harvest` on validated safe plant clusters, or one idempotent simple-meal cook-bill upsert when exactly one cooking workbench is known. `mark_hunt` is eligible only for deterministic low-risk animal batches with exact ids and fresh validation that each target is still present and low-risk; apply designates those animal ids directly, while the rect is only a dashboard/locality hint and a batch sanity bound. Chef-routed food stockpile creation is eligible only after Willie emits a concrete rectangular `create_stockpile_zone` option with the food filter and live validation. Broad bill editing, arbitrary zone edits/deletes, pawn work priorities, and pawn assignment remain outside the Chef apply slice. This wire/persistence shape change has no compat code; wipe-and-regen on upgrade. In Auto, actions become inputs to the deferred planner/Labor/RIMAPI path, while flag requests remain the cross-minister coordination signal.

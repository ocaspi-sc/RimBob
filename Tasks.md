...
- [ ] rimapi-room-reads-live-verify [2026-05-30] #rimapi #construction #willie #live Verify loaded RIMAPI emits the room fields Willie already consumes: hit `/api/v1/map/rooms?include_entry_cells=true&include_cells=true` and `/api/v1/map/region-at`, confirm `entry_cells`, `region_id`, and `contained_building_ids` are present for a real room, then snapshot a live colony and confirm Willie anchors populate `EntryCells` and `ContainedBuildingIds`. [plan](.plans/willie-anchor-detection-closeout.md)

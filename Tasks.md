...
<!-- entries go here -->
- [ ] willie-freezer-apply-readiness [2026-05-30] #willie #apply #readiness Remove hardcoded `ApplyReady=Blocked` in PlacementSolver and wire `ColonyState.StoredResources.CountByDef` into `MaterialsOnHand` so freezer options show real material readiness and an enabled Apply button. [plan](.plans/willie-freezer-apply-readiness.md)

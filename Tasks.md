...
- [ ] willie-solver-tab [2026-05-30] #dashboard #willie #solver #ui Add a Willie-only "Solver" dashboard tab exposing the Placement Solver trace that today only reaches replay JSONL. Reuse the already-serialized `PlacementSolverReplayOutput` (status/no-fit/4 readiness/full `PlacementTrace`): add an in-memory latest-only `WillieSolverStore` + `GET /api/ministers/willie/solver/latest` (mirror food crop-math/hunt-risk endpoints), then a phased tab — v1 solve summary + `NoFitReason`→stage pipeline funnel + readiness ladder; v2 draft-trace + score breakdown; v3 anchor resolution + spec echo. Render-only, no new solver logic. [plan](.plans/willie-solver-tab.md)

...
- [x] willie-options-preserve-offline [2026-05-31] #willie #offline #advice #construction No-clobber guard: when the placement solver can't reach RIMAPI (`RimApiConnectionFailure` / no-map), `MinisterOfWillie` skips republishing degraded no-options advice, so the already-wired boot hydrate keeps showing the last solved Willie `options[]` offline. Salvages the down-detector + no-map 503 fix + trace-note from unlanded branch `codex/willie-offline-snapshot-advice` (rejects its CabinetCycle offline re-run, which can't validate offline anyway). Pivot from advice-restore-from-corpus. [plan](.plans/willie-options-preserve-offline.md)
- [x] restore-minister-outputs-from-corpus [2026-05-31] #willie #replay #offline #flags #advice #construction Restore last-recorded **flags (building_requests)** + advice/options from the corpus (`logs/replay/{minister}-*.jsonl`) into `FlagChannel` + `AdviceBus` at boot, mirroring `ColonySnapshotRestoreHostedService`. Fixes "Willie can't use the request made last turn": `FlagChannel` is in-memory-only (`Program.cs:136`), so Chef's `building_request` is lost on restart / absent on a Willie-only trigger. Shared `ReplayCorpusOutputReader` + restore hosted service(s) + TTL re-stamp on restore + skip-if-live guard. Absorbs the deferred advice-restore-from-corpus; complements willie-options-preserve-offline (don't-clobber going forward vs restore-at-boot). [plan](.plans/restore-minister-outputs-from-corpus.md)
- [x] willie-apply-ignore-materials [2026-05-31] #willie #apply #solver #construction Decoupled `apply_ready` from `materials_ready` so a material-short colony can still place a validated blueprint group (blueprints cost zero materials; pawns haul on build). Solver success path sets `ApplyReady = PlacementValid` instead of deriving from stock; NoFit stays Blocked; `materials_ready` stays advisory (pill + rationale). Reverses `placement-solver.md:166` and overrides `willie-freezer-apply-readiness` Part A. No frontend/RIMAPI change; live validate gate is materials-free. Fixes the live "Apply not wired" repro. [plan](.plans/willie-apply-ignore-materials.md)
- [x] willie-solver-tab [2026-05-30] #dashboard #willie #solver #ui Added latest-only Willie Placement Solver diagnostics: `/api/ministers/willie/solver/latest`, a Willie-only Solver tab with request summary, no-fit pipeline funnel, readiness ladder, launcher/deep-link wiring, and docs/tests for the live store. [plan](.plans/willie-solver-tab.md)
- [x] willie-freezer-apply-readiness [2026-05-30] #willie #apply #readiness Removed the stale `ApplyReady=Blocked` freezer solver stub, wired `ColonyState.StoredResources.CountByDef` into placement `MaterialsOnHand`, gated Willie blueprint apply payloads when solver readiness is material-blocked, and refreshed the apply test fake for `/api/v1/map/work-tables`. [plan](.plans/willie-freezer-apply-readiness.md)
- [x] willie-nonfreezer-solver-wiring [2026-05-30] #willie #solver #construction Generalize `MinisterOfWillie`'s solver gate (`ShouldRunPlacementSolver`/`EnrichFreezerAdvice`/`NoFitNote`) and `Rules.cs`'s freezer-only inbound rule + prose-only missing-room decisions so any building-request class (and kitchen/hospital/storage gaps) drive the Placement Solver and attach validated options. Templates already exist (placement-solver-3). Shares `MinisterOfWillie.cs` with willie-freezer-apply-readiness → land **after** it. Landed `8d66711` (after freezer `f721618`); added starter `KitchenTemplate.cs` + DI. [plan](.plans/willie-nonfreezer-solver-wiring.md)
- [x] willie-multifunction-anchors [2026-05-30] #construction #willie #anchor #ingestion #rimapi Landed work-table ingestion into the shared building registry and multi-function Willie room anchors, so a barracks-with-stove keeps its Barracks primary anchor and also yields a Kitchen anchor for freezer placement. No RIMAPI fork change. [plan](.plans/willie-multifunction-anchors.md)
- [x] willie-anchor-detection-closeout [2026-05-30] #construction #willie #anchor #rimapi #docs Closed out Room/anchor detection: flipped stale `HasReachability:false`, retired anchor-level `RegionId` while keeping the raw `RoomRecord` mirror, added the live-verify capture, and reconciled stale meta-plan/code-review docs. No new RIMAPI endpoints. [plan](.plans/willie-anchor-detection-closeout.md)
...
- [x] willie-build-queue-tab [2026-05-29] #dashboard #willie #ui Landed Willie "Build Queue" dashboard tab with stacked collapsible Requested / Proposed / Placed sections, SVG footprint thumbnails from `AdviceOption.blueprint_group`, readiness/material chips, placed-build table reuse, launcher/deep-link wiring, and explicit unsupported Apply state when no backend option action payload exists. [plan](.plans/willie-build-queue-tab.md)
- [x] willie-option-apply-payload [2026-05-29] #assisted #apply #willie Emitted per-option `place_blueprint_group` action apply payloads and replaced the refusing executor with the real player-click validate -> place -> readback path. Defaults: `require_all=true`, RIMAPI `placement_order=default`, `MaxBlueprintGroupAssets=64`. [plan](.plans/willie-option-apply-payload.md)
- [x] willie-briefing-hud [2026-05-29] #dashboard #willie #ui Replaced the raw-JSON Willie briefing render with a HUD: promoted the existing Willie readout to primary, added an 8-concern tile strip (severity tone) + data-coverage signal bars, and demoted JsonTree groups to a collapsed "Raw briefing" disclosure (Willie branch only). MVP signal bars use the current bool; 3-tier have/need-fork/defer needs a wire change (follow-up). Minimap + gauges + solver-trace filmstrip are follow-ups; minimap shares a base-map render component with Build Queue D. [plan](.plans/willie-briefing-hud.md)
...
- [x] willie-reuse-footprint-review-fixes [2026-05-30] #construction #willie #solver #review Follow-up to the `75c2e1c` S3d land review (and partial fix pass `f9e5d4d`): RF1 hoist loop-invariant `InteriorOrigins`/shell-asset filter out of the generator inner loops; RF2 dedupe by cell-set inside `ReuseExistingFootprintGenerator` so anchor-dup drafts don't consume `MaxDrafts` before `DraftDedupe` (fixes footprint starvation); RF3 parameterize `GetRoomsAsync` detail flags (explicit, default unchanged); RF-T reuse-generator coverage (freezer exclusion, MaxDrafts cap, occupied-cell skip, multi-anchor dedupe, starvation). Finding #3 (generator default templates) already closed by `f9e5d4d`. [plan](.plans/willie-reuse-footprint-review-fixes.md)
...
- [x] rimapi-room-reads-live-verify [2026-05-30] #rimapi #construction #willie #live Verified live: kitchen anchor (FueledStove 44710, centroid {93,0,186}) resolved L3, full L1→L6 chain passed, 2 validated 36-asset "near kitchen" options attached to freezer advice; §4 browser snapshot superseded by JSON evidence (willie snapshot + replay corpus) per operator decision. [plan](.plans/rimapi-room-reads-live-verify.md)
...
- [x] rimapi-building-detail-read [2026-05-24] #rimapi #construction #state Add detailed building read support for Willie: hp/max hp, stuff, room id, working state, power/fuel/flickable state, and flammability. Fork enriches `MapHelper.GetMapBuildings` + `BuildingHelper.BuildingToDto` (and fixes the `BuildingService:37` overwrite bug); RimBob ingests via `MapDto.BuildingDto` + `MapAggregateMapper.BuildingRecordFrom` into the existing `BuildingRecord` placeholder fields. Independent of the Willie orchestrator slices. Landed fork `34d92d0` + RimBob `b4206d4` (`:37` bug fixed). [plan](.plans/rimapi-building-detail-read.md)

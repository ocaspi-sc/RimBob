...
- [ ] welfare-llm-slice-c [2026-06-02] #welfare #llm #rag #escalation #slice **Welfare LLM Slice C.** Give Welfare an LLM escalation path (Chef parity, full feeder). Mirrors `Chef.cs` **minus the first-cycle bootstrap** (removed from contract). WC1 Rules escalation as terminal fallthrough — escalate `unexplained_mood_pressure` only when no wired concern matched but material out-of-scope mood pressure remains (else `needs_stable`); WC2 `welfare.system.md` prompt; WC3 `MinisterOfWelfare` LLM arms (`ForceLlm`/`RulesOnly`, no bootstrap) + `LlmClient`/RAG deps; WC4 `WelfareLlmResponseParser`/normalizer (closed 4-concern map, flags `RequestedFrom:"Willie"`, strip model Apply); WC5 `WelfareRagRetriever`+query builder over mood/room guides; WC6 registry flip (`CanRunLlm`/`HasPrompt`/`HasRag`/`HasRawLlmOutput`/`HasManualLlmOutput`, full views)+DI; WC7 escalation/normalization/replay tests. **Enum stays 4** — LLM frames social/ideology/temperature under nearest concern; promoting them to first-class concerns is evidence-based Slice D. Gimp C1(escalate seam)→C2(prompt+normalize)→C3(RAG+activation). [plan](.plans/welfare-llm-slice-c.md)

...
<!-- entries go here -->
- [ ] willie-multifunction-anchors [2026-05-30] #construction #willie #anchor #ingestion #rimapi Support multi-purpose rooms: a barracks-with-stove must yield a `RoomClass.Kitchen` anchor. Two stacked gaps — (A) work-table buildings (stove/butcher/crafting spots) never reach `state.Buildings` because RimBob ingests only `/api/v1/map/buildings` (excludes work tables); add `GetWorkTablesAsync` + merge `/api/v1/map/work-tables` into the building registry; (B) `WillieAnchorInventoryDerivation` short-circuits on the room's primary role (`FromRoleLabel ?? FromContainedBuildings`), so emit one anchor per detected function (role class + each work-bench class), bench-cell centroid, deduped by (RoomId,Class). No RIMAPI change. Unblocks `rimapi-room-reads-live-verify` L3 for multi-purpose colonies. [plan](.plans/willie-multifunction-anchors.md)

...
- [x] placement-solver-3 [2026-05-29] #construction #willie #solver Room-class breadth for the Placement Solver: extract IRoomTemplate seam + shared shell geometry behind FreezerTemplate (S3a, behavior-identical), add hospital/bedroom/workshop/storage templates + per-measure capacity sizing + RoomClass alias map (S3b), build_order_safety score component (S3c), ReuseExistingFootprintGenerator (S3d, RIMAPI-evidence-gated). S3a–S3c landed `7e3be6f`; S3d deferred (see willie-reuse-footprint). [plan](.plans/placement-solver-3.md)
- [ ] willie-build-queue-tab [2026-05-29] #dashboard #willie #ui Dashboard "Build Queue" Willie view tab: B kanban lanes (Requested / Proposed / In-backlog) + C option-thumbnail cards (footprint render + readiness pills + Apply). Reuses scopes.ts/ministerViewRegistry tab seam, AdviceOption.blueprint_group, construction backlog, existing apply path. Variants A (flat list) + D (minimap overlay) are follow-ups; D shares a base-map render component with the HUD minimap. [plan](.plans/willie-build-queue-tab.md)
...
- [ ] willie-reuse-footprint [2026-05-29] #construction #willie #solver Deferred Solver3 S3d: ReuseExistingFootprintGenerator (expand/repair/repurpose existing rooms). Gated on room-footprint evidence — needs `rimapi-room-entry-cells` (per-room boundary/cells) before there is a footprint to expand; occupancy is point-approx today. Confirm blocker, then ship expand-only first. [plan](.plans/placement-solver-3.md)
...
- [ ] rimapi-map-region-at [2026-05-27] #rimapi #construction #pathfinding #willie Add `GET /api/v1/map/region-at?map_id=...&x=...&z=...` returning the `Region.id` for the cell (deferred follow-up from `rimapi-map-reach-and-path-cost` §"Follow-Up Boundaries"). Cheap wrapper around `map.regionGrid.GetValidRegionAt_NoRebuild`. Willie `AnchorInventory.RegionId` is first consumer; without it the solver still works via FORK3 batch path-cost but cannot pre-cluster candidates by region. [plan](.plans/willie-briefing-schema.md)
- [ ] rimapi-room-entry-cells [2026-05-27] #rimapi #construction #willie #welfare Extend `rimapi-room-detail-read` (or land sibling endpoint) to emit per-room boundary door/door-opening cells. Source for Willie `AnchorInventory.EntryCells[]` (FORK3 batch path-cost `to` set); Welfare may also read for a future `missing_door` rule (basic_shelter moved to Welfare per willie-advice-types §4.5). Fold into `rimapi-room-detail-read` if its scope already covers boundary cells. [plan](.plans/willie-briefing-schema.md)

...
- [ ] willie-home-area-anchor [2026-06-01] #construction #willie #anchor #ingestion #solver **Home-area buildable-region fallback anchor.** Fix live "no anchors": a colony with a painted Home area but no recognized rooms dead-ends at `NoFitReason.NoAnchors` because the anchor inventory derives from `state.Rooms` only. Ingest the Home area (already arriving in the discarded `/map/zones` "+areas" rows) into a light new `MapAreaRegistry` aggregate, emit a low-priority `RoomClass.BuildableRegion` anchor, and have the solver fall back to it **only** when `ResolveNear` finds zero room anchors (fallback-only = the "low priority", no score-weight change). Bounds-clamped placement; cell-mask deferred to buildability-layers. Gate: verify the live Home-area `/map/zones` `type` string first (JSON-first). No RIMAPI change if the row exists. [plan](.plans/willie-home-area-buildable-region-anchor.md)

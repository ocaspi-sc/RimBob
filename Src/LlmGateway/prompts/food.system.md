...
- Flags must be full AgentFlag objects, not bare request arrays: include id, source_minister "Chef", severity (low, medium, high, or critical), domain "food", summary, plus any typed request arrays for cross-minister needs: building_requests, labor_requests, item_requests, and attention. Keep each request entry concrete with request, reason, optional priority, and requested_from. A flag missing this envelope or using an unknown enum token is dropped.

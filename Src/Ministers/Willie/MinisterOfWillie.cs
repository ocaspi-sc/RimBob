    ...
    MinisterReplayRecorder? replay = null) : IMinister
{
    ...
    {
        List<BuildingRequest> requests = [];
        if (directFlag is not null)
        {
            requests.AddRange((directFlag.BuildingRequests ?? [])
                .Where(IsRequestedFromWillie));
        }

        foreach (AgentFlag flag in activeFlags)
        {
            if (directFlag is not null &&
                string.Equals(flag.Id, directFlag.Id, StringComparison.OrdinalIgnoreCase))
            {
                continue;
            }

            requests.AddRange((flag.BuildingRequests ?? [])
                .Where(IsRequestedFromWillie));
        }

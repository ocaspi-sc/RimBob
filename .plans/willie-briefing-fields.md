...
| Per named anchor: `region_id` | `/api/v1/map/region-at` (NEW capture, see schema S5) | `need-fork` | solver region-tier bulk rank pre-A* tiebreak | Empty in Slice A; FORK3 path-cost batch internally uses regions even without exposing the id, so anchor records can ship without `region_id` populated for first Solver1 cut. |

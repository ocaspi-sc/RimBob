...
- `Src/Tests/Willie/FreezerTemplateTests.cs` — 200 food → 5×5 shell with correct cell enumeration; deterministic.
- `Src/Tests/Willie/AnchorResolverTests.cs` — fixture briefing with one kitchen anchor → one resolved anchor; no kitchen → empty.
...
- **Willie Rules wiring** that *calls* the solver on an active `building_request` — owned by [`willie-rules-slice-a.md`](willie-rules-slice-a.md); Solver1 exposes `SolveAsync`, Rules wires it (the freezer rule's solver path is a TODO in Rules Slice A until Solver1 lands).

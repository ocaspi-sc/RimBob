...
| `room_class` | enum (`RoomClass`)? | – | If a *named functional room* is needed (`freezer`, `hospital`, `kitchen`, `workshop`, `research`, `bedroom`, `barracks`, `prison`, `recreation`, `storage`). Drives `functional_rooms` (survival-floor bedroom/barracks asks fold here since `basic_shelter` moved to Welfare — see `willie-advice-types.md` §4.5). |
...
| **Defense** | Walls / doors / barricades / embrasures | `building_requests` → `target_class:wall`/`door`/`barricade`/`embrasure` | `fire_risk` (material + spacing); enclosure-shell asks fold into `functional_rooms` (`basic_shelter` retired — see `willie-advice-types.md` §4.5) |
...
| **Welfare** | Bedrooms / barracks (survival floor + quality) | `building_requests` → `room_class:bedroom`/`barracks`, `capacity_need:{occupants|beds}`, `target_class:bed` | `functional_rooms` (single concern — survival-floor and quality both routed here since `basic_shelter` moved to Welfare 2026-05-27) |
